...
### 3.1.3 门店（产品需求细化） [x]
状态：门店详情（证照、导航/拨号、门店商品映射）功能完成（wx-fe 最小版已接入并落地）。
...
- 优惠券 [x]
  ...
  - 来源拆分最小版已完成：在「我的-优惠券」页已按来源拆分平台券/门店券 Tab；结算页支持查询“本单可用优惠券”并进行最小版选择，详细接口与适用范围说明见 `3.2.6.1 小程序优惠券接口约定`。
- 客服 [x]
...
状态：
- 我的-钱包/积分（只读）功能完成（wx-fe 最小版已接入并落地）。提现/账单/提现账户与积分明细/积分商品将按迭代推进。
- 我的-分享（最小版）功能完成：支持在「我的 → 分享推广」生成带推荐人ID与可选门店ID的分享链接与二维码海报；`app.tsx` 在启动时解析 `referrer_id` 并调用 `/api/v1/referrals/bind` 记录推荐闭包关系。后续将完善海报模板与小程序码生成。
- 设置 [x]
  - 账户管理（修改手机号、修改密码、注销）、隐私协议/用户协议、关于我们等静态内容。

补充说明（2026-01-05）：

- 意见反馈：wx-fe 已上线「意见反馈」页面与入口（设置页与个人中心均可进入）。提交成功后会弹窗展示工单编号（Ticket ID），便于用户与客服沟通定位问题。（实现基于 `POST /api/v1/feedback`，若后端未提供则回退至 `POST /api/v1/tickets`）
