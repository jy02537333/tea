...
- 客服 [x]
  - 展示客服电话，支持一键拨打；意见反馈入口（文本 + 图片）与常见问题列表。
- 分享 [x]
...
目标：为平台运营与客服团队提供统一的用户问题与投诉处理入口，将「客服电话/意见反馈/投诉建议」抽象为工单系统，打通消息通知与数据看板中的待办统计。

功能点：

- 工单来源与类型
  - 来源：小程序端意见反馈表单、订单/售后页面发起的投诉、客服电话记录转工单、门店后台转交问题等；
  - 类型：咨询类（功能咨询、使用问题）、订单类（发货/配送/售后）、充值/财务类、投诉建议类（服务态度、产品质量等）；
  - 每条工单需记录来源渠道、关联用户、关联订单/门店（如有）、提交时间与当前状态。
- 工单生命周期（状态机）
  - 典型状态：`new`（新建）、`pending`（处理中）、`waiting_customer`（待用户补充）、`resolved`（已解决）、`closed`（已关闭）、`rejected`（驳回/无效）；
  - 支持状态流转记录与操作人记录（客服/运营账号），必要时可追加「升级处理人」字段（如升级到主管/法务）。
- 工单处理与记录
  - 后台列表：按状态、类型、时间、用户、订单号、门店、处理人等维度筛选；
  - 工单详情：问题描述、附件（截图等）、处理记录时间线（每次回复、备注、状态变更、内部备注）；
  - 支持内部备注与对用户可见回复分离，避免内部沟通泄漏给用户；
  - 可配置模板回复（常见问题），提高处理效率。
- 投诉建议专属处理
  - 对严重投诉可打上「严重/高优先级」标签，并要求必须由指定角色审批后才能关闭；
  - 投诉类工单支持关联门店与门店管理员，平台可决定是否抄送门店查看记录；
  - 为后续质量分析提供标签维度（如投诉原因、责任方判定等）。
- 通知与待办联动
  - 与首页 `3.2.0 后台首页与总览` 的「待办」模块联动，展示待处理工单数量；
  - 支持按客服专员分配工单（自动或手动），并在其登录后在顶部通知中看到分配情况；
  - 必要时可对即将 SLA 超时的工单发出预警提醒。

关键接口示例：

- `GET /api/v1/admin/support/tickets` — 工单列表；
- `GET /api/v1/admin/support/tickets/{id}` — 工单详情；
- `POST /api/v1/admin/support/tickets/{id}/reply` — 工单回复（区分用户可见与内部备注）；
- `POST /api/v1/admin/support/tickets/{id}/assign` — 分配/转派工单；
- `POST /api/v1/admin/support/tickets/{id}/status` — 更新工单状态。

验收要点：

- 小程序端的意见反馈、投诉入口能够在后台形成工单记录，包含必要的用户与上下文信息；
- 客服在后台可基于列表/筛选快速定位工单，并完整记录处理过程；
- 工单状态与首页待办数、数据看板统计保持一致，严重投诉有额外标识与审批要求。
...
- 商品分类（服务商品/外卖商品/其他） [] — 实现时关注：PRD 已给出方向，但门店侧具体分类字段与分类子集需要在实现阶段细化。
...
- 财务提现（提现申请） [] — 实现时关注：门店侧走「结算单 + 提现/结算申请」流程，需要在实现中补充审核、打款与对账细节。
...
### 3.3.8 门店营销与活动管理（补充说明） []

目标：补齐门店后台在需求文档 5.3 中描述的「活动管理/优惠券」能力说明，在不重复平台营销引擎的前提下，明确门店可发起的本店活动/优惠券操作与平台审核边界。

功能点：

- 门店活动管理
  - 活动类型：门店专属活动（如「本店满减」「到店试饮活动」「线下沙龙」等），底层复用平台活动模型；
  - 创建流程：门店在后台填写活动基本信息（名称、时间、海报、方式、地址/链接、主理人、联系人、人数限制、报名费用、报名条件、报名信息设置、退费规则等），提交为「草稿」或「待审核」状态；
  - 审核机制：平台运营在平台端活动管理中查看门店提交的活动，进行审核通过/驳回；通过后活动对用户可见并支持报名/支付；
  - 报名管理：门店可查看本店活动的报名列表、导出报名名单、处理退款申请（按退费规则），退款走平台统一订单/财务流水。
- 门店优惠券管理
  - 能力边界：门店可基于平台统一的优惠券模型创建「门店专属优惠券」，在适用范围中限定为「本门店/本门店商品」；
  - 创建与审核：门店创建优惠券草稿（名称、图片、使用规则、权益介绍、有效期、针对用户群、数量等），提交平台审核；通过后即可由门店在本店会员中发放或用户自助领取；
  - 使用与统计：门店可查看优惠券领取数、使用数、带来的订单与销售额；平台可在全局视角统计门店券使用情况。
- 与平台营销的关系
  - 所有活动与优惠券的底层规则与数据结构由平台营销模块（`3.2.6 营销管理`）统一控制，门店只是「提交申请」「配置本店范围」，避免规则分裂；
  - 平台可对门店活动/优惠券设置配额与权限（哪些门店/角色可创建活动、最高补贴额度等）。

关键接口示例：

- `GET /api/v1/stores/{store_id}/campaigns` — 门店可见的活动列表；
- `POST /api/v1/stores/{store_id}/campaigns` — 门店创建活动草稿；
- `GET /api/v1/stores/{store_id}/campaigns/{id}/registrations` — 活动报名列表；
- `POST /api/v1/stores/{store_id}/coupons` — 门店创建优惠券草稿（提交平台审核）；
- `GET /api/v1/stores/{store_id}/coupons/stats` — 门店优惠券使用统计。

验收要点：

- 门店能够创建并提交本店活动/优惠券的配置，平台能在统一后台看到并进行审核控制；
- 门店活动的报名/退费流程与平台订单、财务模块打通，数据可对账；
- 门店优惠券严格受平台营销模型与风控约束，门店无法绕开平台规则自行发券导致风控漏洞。

关键接口（示例）：

- `GET /api/v1/stores/{store_id}/dashboard/summary` — 门店经营摘要数据；
- `GET /api/v1/stores/{store_id}/dashboard/todos` — 门店待处理事项列表（未接单订单、即将超时订单、售后待处理等）；
- `POST /api/v1/stores/{store_id}/pos/orders` — 收银台创建线下订单接口，支持多种支付方式与组合支付参数。

验收要点：

- 门店登录后能在首页看到与 5.3 描述一致的菜单/导航结构与经营数据；
- 经营参谋中各维度经营指标与门店订单/财务模块中的数据一致，切换时间维度后能正确刷新；
- 工作台中的待处理订单列表和收银入口，可以支撑门店日常运营的 80%+ 高频操作。
