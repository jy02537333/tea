...
| [✅] | 我的-钱包 | 余额、茶币、账单、提现、银行卡管理 | `doc/prd.md` → `3.1.4 我的`（钱包）+ 分销结算设计 | 功能完成 | wx-fe 已实现“钱包”只读展示（余额/茶币/冻结金额）；提现/账单/账户管理按后续迭代推进 |
| [✅] | 我的-积分 | 积分余额、积分记录、积分商品展示 | `doc/prd.md` → `3.1.4 我的`（积分中心）+ `3.2.6 营销管理`（积分规则） | 功能完成 | wx-fe 已实现“积分中心”只读展示（积分余额）；积分明细与积分商品按后续迭代推进 |
| [✅] | 我的-优惠券 | 待使用/已使用/已过期 | `doc/prd.md` → `3.1.4 我的`（优惠券）+ `3.2.6.1 小程序优惠券接口约定` | 功能完成 | 小程序「我的-优惠券」页已实现来源拆分（平台券/门店券 Tab）并调用 `GET /api/v1/user/coupons` 展示账户可用券；结算页支持查询“本单可用优惠券”并最小版选择（`POST /api/v1/orders/available-coupons`）。PRD 中来源拆分早前标注为规划中，现以最小版完成；后续详细接口与适用范围在 3.2.6.1 继续完善。 |
| [✅] | 我的-客服 | 客服电话、意见反馈、帮助文档 | `doc/prd.md` → `3.1.4 我的`（客服） | 功能完成 | 小程序「我的」页提供“联系客服 / 意见反馈 / 帮助文档”入口；意见反馈页支持选择类型并点击“提交”创建工单（优先 `POST /api/v1/feedback`，未提供时回退 `POST /api/v1/tickets`）；提交成功弹窗展示工单编号（Ticket ID）；订单详情亦支持投诉提交；后台侧在 `/api/v1/admin/tickets` 统一处理与跟进 |
| [✅] | 我的-分享 | 分享海报（最小版：二维码海报预览/保存）、绑定推荐关系（最小版：入参解析+后端闭包记录） | `doc/prd.md` → `3.1.4 我的`（分享）+ `3.4 分销系统` | 功能完成 | wx-fe 支持生成带 `referrer_id` 与可选 `store_id` 的分享链接并生成二维码海报；`app.tsx` 解析入参并调用 `/api/v1/referrals/bind` 记录推荐闭包（depth=1）；后续将完善海报素材模板与小程序码生成 |
