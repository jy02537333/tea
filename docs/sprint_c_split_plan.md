...
### SC6a: 接受/拒绝真实状态迁移与最小测试
- 后端：
  - `OrderService.AdminAcceptOrder(orderID)`：将状态由已付款(2)迁移为处理中(3)
  - `OrderService.AdminRejectOrder(orderID, reason)`：复用退款逻辑，置为已取消(5)、支付已退款(4)
  - `OrderHandler.AdminAccept/AdminReject`：调用服务层，并写入 before/after 操作日志
- 路由：
  - `POST /api/v1/admin/orders/:id/accept`
  - `POST /api/v1/admin/orders/:id/reject`
- 测试：
  - 新增 `internal/service/order_accept_reject_test.go`（SQLite 内存库），覆盖 2→3 与 2→5/4 迁移
- 备注：后续按小 PR 推进打印任务持久化与状态机

#### 规则细化与后续计划
- 拒绝（AdminReject）：仅当订单处于处理中/配送中(3) 时允许，其他状态报错；执行路径复用退款逻辑，结果为已取消(5)+已退款(4)。
- 接受（AdminAccept）：仅当订单已付款(2) 时允许，迁移为处理中(3)。
- 佣金回滚：已在 AdminRefund（作为 AdminReject 的退款路径）与 `AdminRefundConfirm` 成功后调用 `ReverseOrderCommissions` 对该订单的未提现佣金进行回滚；若回滚失败不阻塞退款，会记录操作日志供财务后续处理（见财务报表模块）。
