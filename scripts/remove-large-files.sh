...
# 文件用途：
# - 该脚本用于“当前分支”的历史重写，移除超过指定阈值（默认 100M）的所有大文件 blob。
# - 适用于功能分支或临时分支自助清理，避免 GitHub 100MB 限制导致的 push 阻断。
# - 不会自动切换分支；执行后需使用 --force-with-lease 推送当前分支并通知协作者对齐历史。
# 重要：历史重写为破坏性操作，推送后同事需要重新克隆或硬重置。
