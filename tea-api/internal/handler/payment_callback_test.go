...
func Test_PaymentCallback_SignatureFailure(t *testing.T) {
    ...
    json.NewDecoder(resp2.Body).Decode(&catResp)
    resp2.Body.Close()
    if catResp.Data.ID == 0 { t.Fatalf("create category failed") }
    ...
    json.NewDecoder(resp3.Body).Decode(&prodResp)
    resp3.Body.Close()
    if prodResp.Data.ID == 0 { t.Fatalf("create product failed") }
    ...
    json.NewDecoder(resp4.Body).Decode(&orderResp)
    resp4.Body.Close()
    if orderResp.Data.ID == 0 { t.Fatalf("create order failed") }
    ...
    var uresp struct{ Code int; Data struct{ PaymentNo string } }
    json.NewDecoder(resp5.Body).Decode(&uresp)
    resp5.Body.Close()
    if uresp.Data.PaymentNo == "" { t.Fatalf("unified-order failed") }
    ...
    var od struct{ Code int; Data struct{ Status int; PayStatus int } }
    json.NewDecoder(resp7.Body).Decode(&od)
    resp7.Body.Close()
...
func Test_PaymentCallback_IdempotentSuccess(t *testing.T) {
    ...
    if resp6.StatusCode != 200 { t.Fatalf("first callback expected 200, got %d", resp6.StatusCode) }
    resp6.Body.Close()
