...
// 验证：给角色新增权限后，服务自动对该角色下用户的权限缓存失效，立刻生效
func Test_RBAC_Mutation_AutoInvalidate(t *testing.T) {
	...
	// 找到 auditor 角色 与 accrual:run 权限
	var role model.Role
	...
	// 创建一个审计员用户并绑定角色（先清理该角色上的 run 权限）
	var rp model.RolePermission
	_ = db.Unscoped().Where("role_id = ? AND permission_id = ?", role.ID, pRun.ID).Delete(&rp).Error

	u := model.User{BaseModel: model.BaseModel{UID: "u-auto-inv"}, OpenID: "auto_inv_openid", Phone: "auto_inv_phone", Nickname: "aud", Status: 1}
	if err := db.Where("open_id = ?", u.OpenID).FirstOrCreate(&u).Error; err != nil {
		t.Fatalf("create user: %v", err)
	}
	var ur model.UserRole
	_ = db.Where("user_id = ? AND role_id = ?", u.ID, role.ID).FirstOrCreate(&ur, &model.UserRole{BaseModel: model.BaseModel{UID: "ur-auto-inv"}, UserID: u.ID, RoleID: role.ID}).Error
	...
	// 管理员调用 赋权 接口（应自动失效该角色所有用户的缓存）
	bodyAssign, _ := json.Marshal(map[string]any{"role_id": role.ID, "permission_id": pRun.ID})
	reqAssign, _ := http.NewRequest("POST", ts.URL+"/api/v1/admin/rbac/role/assign-permission", bytes.NewReader(bodyAssign))
	...
	if runResp2.StatusCode != 200 {
		t.Fatalf("expect allowed after auto-invalidate, got %d", runResp2.StatusCode)
	}
