...
**用户管理** `#/users`
	- [x] 页面骨架与路由集成（已 scaffold `src/pages/Users/index.tsx`，已加入 AppRouter）
	- [x] 服务层 typescript 类型与接口（已 scaffold `src/services/users.ts`）
	- [x] 用户表格、详情、编辑弹窗开发，已对接 API，支持基础数据展示
	- [ ] 编辑、禁用、分页等交互细节开发中
	- [x] 文档与代码同步推进
...
- **商品管理** `#/products`
	...
	- [x] 支持图片上传（已对接阿里云 OSS，需后端 `/api/v1/oss/signature` 签名接口）
	- [ ] 需后端提供 OSS 图片删除 API（如 `/api/v1/oss/delete`），支持前端商品图片管理删除
	- [ ] 建议后端统一图片上传、删除、鉴权策略，便于前端安全调用
- **分类管理** `#/categories`
...
**项目目录建议（骨架）**
- `admin-fe/`
	- `index.html`：Vite 入口 HTML，包含 `<div id="root"></div>` 和 `<script src="/src/main.tsx">`
	- `src/main.tsx`：React 应用入口，集成 `react-router-dom` 路由，支持多页面切换
	 ...
	 - 路由建议：
		 ...
		 - `/stores` → Stores
		 - `/login` → Login
	 - 可通过顶部导航栏或侧边栏实现页面跳转，推荐使用 `react-router-dom` 的 `<Routes>` 和 `<Link>` 组件。
	- `src/components/`：DataTable, CrudForm, PermissionTree, Uploader
	- `src/services/`：`api.ts`, `auth.ts`, `products.ts`, `orders.ts`
	- `wx-fe/`（Taro/uni-app）
		- `src/app.tsx`：Taro 应用入口，负责全局样式与页面渲染
		- `src/app.config.ts`：Taro 路由配置，pages 数组首项为首页 index，后续依次为各业务页面
		- `src/pages/index.tsx`：首页入口页面（示例）
		...
		- 路由建议：
			...
			- `/pages/coupons` → 优惠券
			- `/pages/stores` → 门店列表
		- 页面间通过 Taro 的 `navigateTo`、`switchTab`、`redirectTo` 等 API 跳转。
